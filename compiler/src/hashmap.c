...
int string_equals(const void *a, const void *b) { return strcmp((const char *)a, (const char *)b) == 0; }

int ptr_equals(const void *a, const void *b) { return a == b; }
...
// Create hashmap
hashmap_t *hashmap_create(size_t init_capacity, hash_func hf, equals_func eq) {
...
static void free_bucket(entry_t *e, void (*destroy_key)(void *), void (*destroy_val)(void *)) {
	while (e) {
		entry_t *next = e->next;
		if (destroy_key)
			destroy_key(e->key);
...
// Destroy hashmap
void hashmap_destroy(hashmap_t *map, void (*destroy_key)(void *), void (*destroy_val)(void *)) {
...
// Check existence
int hashmap_contains(const hashmap_t *map, const void *key) { return hashmap_get(map, key) != NULL; }

// Remove
int hashmap_remove(hashmap_t *map, const void *key, void **out_key, void **out_val) {
	size_t idx = map->hf(key) & (map->capacity - 1);
	entry_t **pp = &map->buckets[idx];
	while (*pp) {
		if (map->eq((*pp)->key, key)) {
...
// Size
size_t hashmap_size(const hashmap_t *map) { return map->size; }
...
hashmap_t *hashmap_copy(const hashmap_t *orig, copy_func copy_key, copy_func copy_val) {
	hashmap_t *new_map = hashmap_create(orig->capacity, orig->hf, orig->eq);
	for (size_t i = 0; i < orig->capacity; i++) {
		entry_t *e = orig->buckets[i];
		while (e) {
			void *k = copy_key ? copy_key(e->key) : e->key;
			void *v = copy_val ? copy_val(e->val) : e->val;
			hashmap_put(new_map, k, v);
			e = e->next;
		}
	}
	return new_map;
