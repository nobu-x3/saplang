...
fn void write_role(io::OutBuf* out, cfg::Cfg* g, cfg::BasicBlock* block) {
    ...
    if(block.term.kind == cfg::TermKind::Unreachable) { io::outbuf_write(out, " (unreachable)"); }
}

fn void write_terminator(io::OutBuf* out, cfg::Terminator* t) {
    cfg::TermKind kind = t.kind;
    if(kind == cfg::TermKind::Goto) {
        io::outbuf_write(out, "goto bb");
        io::outbuf_write_u64(out, (u64)t.goto_target);
    } else if(kind == cfg::TermKind::CondBranch) {
    ...
    } else if(kind == cfg::TermKind::Switch) {
    ...
    } else if(kind == cfg::TermKind::Return) {
