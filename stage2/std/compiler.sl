...
export fn bool set_validated_target(Compiler* c, const u8[] t) {
    if(slice_eq(t, "linux") || slice_eq(t, "windows")) {
        c.target = t;
        return true;
    }
    sys::fprintf(sys::stderr_file(), "unsupported -target %.*s (only 'linux' is supported)\n", (i32)t.len, (i8*)t.ptr);
    return false;
}
...
fn i8** build_link_argv(Compiler* c, const u8[][] object_paths, link_paths::LinkPaths* paths) {
    ...
    u64 n = 0;
    argv[n] = cstr(c.allocator, "clang"); n += 1;
    argv[n] = cstr(c.allocator, "-fuse-ld=lld"); n += 1;
    argv[n] = cstr(c.allocator, "-Wl,/STACK:8388608"); n += 1;
...
fn i8* output_cstr(Compiler* c) {
    if(c.output_path.len == 0) { return cstr(c.allocator, "a.exe"); }
    u64 index = c.output_path.len;
    while(index > 0) {
        index -= 1;
        if(c.output_path[index] == '.') { return cstr(c.allocator, c.output_path); }
