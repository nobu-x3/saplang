...
fn i32 jit_return(arena::Arena* a, const u8[]src, i32 expected, const u8[] msg) {
...
fn i32 jit_ptr_cast_index(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_pointer_arithmetic(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_voidptr_to_typed(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_mixed_int_width_cmp(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_slice_field_write(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_slice_ptr_field_read(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_type_alias(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_anon_struct_type(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_generic_struct(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_generic_struct_param(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_generic_slice(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_pointer_compound(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_rvalue_struct_field(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_rvalue_field_nested(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_compound_assign_widen(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_const_slice_index(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_const_string_slice(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_const_struct_table(arena::Arena* a, const u8[]msg) {
    return jit_return(a, "struct E { const u8[] name; i32 v; } const E[] t = [ {\"ab\", 10}, {\"c\", 20} ]; fn i32 main() { return t[1].v + (i32)t[0].name.len; }", 22, msg);
}

// An enum acts as its base int in arithmetic; the runtime value is the underlying integer.
fn i32 jit_enum_arithmetic(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_enum_unary(arena::Arena* a, const u8[]msg) {
...
fn i32 jit_const_array_string(arena::Arena* a, const u8[]msg) {
...
fn i32 opt_debug_aggregate_and_ssa(arena::Arena* a, const u8[]msg) {
    arena::Arena* ja = fresh_arena(a);
    ...
    if(!testing::expect_eq(test_util::error_count(m), (u64)0, msg)) { return -1; }
    sapir::SapirModule* sm = lower::lower_module(m);
    const u8[] ir = codegen::codegen_ir_string(sm, arena::allocator(ja), codegen::BuildConfig::Debug);
...
fn bool contains(const u8[] hay, const u8[] needle) {
...
fn i32 opt_debug_info(arena::Arena* a, const u8[]msg) {
    arena::Arena* ja = fresh_arena(a);
    ...
    if(!testing::expect_eq(test_util::error_count(m), (u64)0, msg)) { return -1; }
    sapir::SapirModule* sm = lower::lower_module(m);
    const u8[] debug_ir = codegen::codegen_ir_string(sm, arena::allocator(ja), codegen::BuildConfig::Debug);
    const u8[] release_ir = codegen::codegen_ir_string(sm, arena::allocator(ja), codegen::BuildConfig::Release);
...
fn i32 opt_release_mem2reg(arena::Arena* a, const u8[]msg) {
    arena::Arena* ja = fresh_arena(a);
...
fn i32 unsigned_index_zero_extends(arena::Arena* a, const u8[]msg) {
    arena::Arena* ja = fresh_arena(a);
...
fn i32 signed_index_sign_extends(arena::Arena* a, const u8[]msg) {
    arena::Arena* ja = fresh_arena(a);
...
fn i32 jit_mixed_struct_lit(arena::Arena* a, const u8[]msg) {
    return jit_return(a, "struct V { i32 x; i32 y; i32 z; } fn i32 main() { V v = {5, .z = 9, 6}; return v.y * 10 + v.z; }", 69, msg);
}

fn i32 jit_partial_slice_lit(arena::Arena* a, const u8[]msg) {
...
// A string literal carries no type of its own, so it must not drive inference — T comes from the list.
fn i32 jit_literal_into_generic(arena::Arena* a, const u8[]msg) {
    return jit_return(a, "fn Type Box(comptime Type T) { return struct { T value; }; } fn u64 hold(comptime Type T, Box(T)* b, T v) { b.value = v; return b.value.len; } fn i32 main() { Box(const u8[]) b; return (i32)hold(&b, \"hello\"); }", 5, msg);
}

fn i32 jit_sizeof_generic_instantiation(arena::Arena* a, const u8[]msg) {
...
fn i32 main() {
    ...
    testing::add(suite, "jit_literal_into_generic", &jit_literal_into_generic);
