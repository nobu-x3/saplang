...
fn module::Module* with_defines(arena::Arena* a, const u8[]src, module::Define[] defines) {
    module::BuildInfo build = test_util::host_build();
    build.defines = defines;
    return test_util::frontend_build(a, src, build);
}

fn i32 ok_top_level_fn(arena::Arena* a, const u8[]m) {
...
fn i32 ok_else_taken(arena::Arena* a, const u8[]m) {
...
fn i32 ok_no_else_condition_false(arena::Arena* a, const u8[]m) {
    module::Module* mod = test_util::frontend(a, "comprun if (build::os == \"windows\") { fn i32 impl() { return missing_api(); } }\nexport fn i32 f() { return 1; }");
    if(!testing::expect_eq(test_util::error_count(mod), (u64)0, m)) { return -1; }
    return 0;
}

fn i32 ok_else_if_chain(arena::Arena* a, const u8[]m) {
...
fn i32 ok_arch_and_not_equal(arena::Arena* a, const u8[]m) {
...
fn i32 ok_or_and_negation(arena::Arena* a, const u8[]m) {
...
fn i32 ok_struct_alias_and_global(arena::Arena* a, const u8[]m) {
...
fn i32 ok_generic_fn_in_branch(arena::Arena* a, const u8[]m) {
...
fn i32 ok_extern_block_in_branch(arena::Arena* a, const u8[]m) {
...
fn i32 ok_stmt_level(arena::Arena* a, const u8[]m) {
...
fn i32 err_non_build_condition(arena::Arena* a, const u8[]m) {
...
fn i32 err_non_string_operand(arena::Arena* a, const u8[]m) {
...
fn i32 err_defined_without_string(arena::Arena* a, const u8[]m) {
...
fn i32 ok_compinsert_emits_comp_if(arena::Arena* a, const u8[]m) {
...
fn i32 ok_compinsert_stmt_emits_comp_if(arena::Arena* a, const u8[]m) {
    module::Module* mod = test_util::frontend(a, "export fn i32 f() {\n  i32 total = 1;\n  compinsert(\"comprun if (build::config == \\\"Debug\\\") { total = total + 5; } else { total = missing_api(); }\");\n  return total;\n}");
    if(!testing::expect_eq(test_util::error_count(mod), (u64)0, m)) { return -1; }
    return 0;
}

fn i32 err_build_in_comprun_block(arena::Arena* a, const u8[]m) {
...
fn i32 err_build_in_runtime_expr(arena::Arena* a, const u8[]m) {
...
fn i32 err_braceless_branch_stmt(arena::Arena* a, const u8[]m) {
...
fn i32 err_braceless_branch_top_level(arena::Arena* a, const u8[]m) {
...
fn i32 err_exported_comp_if(arena::Arena* a, const u8[]m) {
...
fn i32 main() {
    testing::init();
    const u8[] suite = "E2E Conditional Compilation Tests";
