...
fn i32 straight_line_void(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq(g.blocks.data.len, (u64)2, m)) { return -1; }
    if(!testing::expect_eq((u64)g.entry, (u64)0, m)) { return -2; }
    if(!testing::expect_eq((u64)g.exit, (u64)1, m)) { return -3; }
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)2, m)) { return -4; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Return, m)) { return -5; }
    if(!testing::expect_true(g.blocks.data[0].terminated, m)) { return -6; }
    return 0;
}
...
fn i32 non_void_no_return_is_unreachable(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, func);
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Unreachable, m)) { return -1; }
    return 0;
}
...
fn i32 mixed_leaf_kinds_referenced(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, func);
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)3, m)) { return -1; }
    if(!testing::expect_eq((void*)g.blocks.data[0].stmts.data[0], (void*)stmts[0], m)) { return -2; }
    if(!testing::expect_eq((void*)g.blocks.data[0].stmts.data[2], (void*)stmts[2], m)) { return -3; }
    return 0;
}
...
fn i32 predecessors_and_exit_finalized(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, func);
    if(!testing::expect_eq(g.blocks.data[g.entry].predecessors.data.len, (u64)0, m)) { return -1; }
    if(!testing::expect_eq(g.blocks.data[g.exit].predecessors.data.len, (u64)0, m)) { return -2; }
    if(!testing::expect_true(g.blocks.data[g.exit].terminated, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[g.exit].term.kind, (u64)cfg::TermKind::Unreachable, m)) { return -4; }
    return 0;
}
...
fn i32 empty_void_body(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, func);
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)0, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Return, m)) { return -2; }
    return 0;
}
...
fn i32 nested_blocks_flatten(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, func);
    if(!testing::expect_eq(g.blocks.data.len, (u64)2, m)) { return -1; }
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)2, m)) { return -2; }
    return 0;
}
...
fn i32 if_else_merge(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq(g.blocks.data.len, (u64)5, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::CondBranch, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.then_target, (u64)2, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.else_target, (u64)3, m)) { return -4; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::Goto, m)) { return -5; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.goto_target, (u64)4, m)) { return -6; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)2, m)) { return -7; }
    if(!testing::expect_eq((u64)g.blocks.data[4].term.kind, (u64)cfg::TermKind::Return, m)) { return -8; }
    return 0;
}
...
fn i32 if_no_else(arena::Arena* a, const u8[]m) {
    ...
    body[0] = iff;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq(g.blocks.data[3].stmts.data.len, (u64)0, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.kind, (u64)cfg::TermKind::Goto, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)4, m)) { return -3; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)2, m)) { return -4; }
    return 0;
}
...
fn i32 if_both_return_after_unreachable(arena::Arena* a, const u8[]m) {
    ...
    body[0] = iff;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::Return, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.kind, (u64)cfg::TermKind::Return, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)0, m)) { return -3; }
    return 0;
}
...
fn i32 while_loop_edges(arena::Arena* a, const u8[]m) {
    ...
    body[0] = wh;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Goto, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.goto_target, (u64)2, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::CondBranch, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.then_target, (u64)3, m)) { return -4; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.else_target, (u64)4, m)) { return -5; }
    if(!testing::expect_eq(g.blocks.data[2].predecessors.data.len, (u64)2, m)) { return -6; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)2, m)) { return -7; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)1, m)) { return -8; }
    return 0;
}
...
fn i32 while_break(arena::Arena* a, const u8[]m) {
    ...
    body[0] = wh;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.kind, (u64)cfg::TermKind::Goto, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)4, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data[2].predecessors.data.len, (u64)1, m)) { return -3; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)2, m)) { return -4; }
    return 0;
}
...
fn i32 while_continue(arena::Arena* a, const u8[]m) {
    ...
    body[0] = wh;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)2, m)) { return -1; }
    if(!testing::expect_eq(g.blocks.data[2].predecessors.data.len, (u64)2, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)1, m)) { return -3; }
    return 0;
}
...
fn i32 for_loop_edges(arena::Arena* a, const u8[]m) {
    ...
    body[0] = fr;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)1, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.goto_target, (u64)2, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::CondBranch, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.then_target, (u64)3, m)) { return -4; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.else_target, (u64)5, m)) { return -5; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)4, m)) { return -6; }
    if(!testing::expect_eq((u64)g.blocks.data[4].term.goto_target, (u64)2, m)) { return -7; }
    if(!testing::expect_eq(g.blocks.data[2].predecessors.data.len, (u64)2, m)) { return -8; }
    return 0;
}
...
fn i32 for_no_cond(arena::Arena* a, const u8[]m) {
    ...
    body[0] = fr;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::Goto, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.goto_target, (u64)3, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[4].term.goto_target, (u64)2, m)) { return -3; }
    return 0;
}
...
fn i32 for_continue_targets_post(arena::Arena* a, const u8[]m) {
    ...
    body[0] = fr;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)4, m)) { return -1; }
    return 0;
}
...
fn i32 switch_arms_dispatch(arena::Arena* a, const u8[]m) {
    ...
    body[0] = sw;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Switch, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_default, (u64)3, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data[0].term.switch_arms.len, (u64)2, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_arms[0].target, (u64)4, m)) { return -4; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_arms[1].target, (u64)5, m)) { return -5; }
    if(!testing::expect_eq(g.blocks.data[2].predecessors.data.len, (u64)3, m)) { return -6; }
    if(!testing::expect_eq((u64)g.blocks.data[4].term.goto_target, (u64)2, m)) { return -7; }
    return 0;
}
...
fn i32 switch_no_else_default_falls_through(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_default, (u64)2, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::Return, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data[0].term.switch_arms.len, (u64)1, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_arms[0].target, (u64)3, m)) { return -4; }
    return 0;
}
...
fn i32 switch_break_fresh_continuation(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)2, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[4].term.kind, (u64)cfg::TermKind::Unreachable, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data.len, (u64)5, m)) { return -3; }
    return 0;
}
...
fn i32 return_fresh_unreachable(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Return, m)) { return -1; }
    if(!testing::expect_eq((void*)g.blocks.data[0].term.return_value, null, m)) { return -2; }
    if(!testing::expect_eq(g.blocks.data.len, (u64)3, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[2].term.kind, (u64)cfg::TermKind::Unreachable, m)) { return -4; }
    return 0;
}
...
fn i32 return_value_recorded(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq((void*)g.blocks.data[0].term.return_value, (void*)val, m)) { return -1; }
    return 0;
}
...
fn i32 defer_fallthrough(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)1, m)) { return -1; }
    if(!testing::expect_eq((void*)g.blocks.data[0].stmts.data[0], (void*)s, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.kind, (u64)cfg::TermKind::Return, m)) { return -3; }
    return 0;
}
...
fn i32 defer_reverse_order(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq(g.blocks.data[0].stmts.data.len, (u64)2, m)) { return -1; }
    if(!testing::expect_eq((void*)g.blocks.data[0].stmts.data[0], (void*)sb, m)) { return -2; }
    if(!testing::expect_eq((void*)g.blocks.data[0].stmts.data[1], (void*)sa, m)) { return -3; }
    return 0;
}
...
fn i32 defer_on_break(arena::Arena* a, const u8[]m) {
    ...
    body[0] = wh;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq(g.blocks.data[3].stmts.data.len, (u64)1, m)) { return -1; }
    if(!testing::expect_eq((void*)g.blocks.data[3].stmts.data[0], (void*)s, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)4, m)) { return -3; }
    return 0;
}
...
fn i32 nested_if_in_while(arena::Arena* a, const u8[]m) {
    ...
    body[0] = wh;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.kind, (u64)cfg::TermKind::CondBranch, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[7].term.kind, (u64)cfg::TermKind::Goto, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[7].term.goto_target, (u64)2, m)) { return -3; }
    return 0;
}
...
fn i32 continue_through_switch(arena::Arena* a, const u8[]m) {
    ...
    body[0] = wh;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[6].term.kind, (u64)cfg::TermKind::Goto, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[6].term.goto_target, (u64)2, m)) { return -2; }
    return 0;
}
...
fn i32 switch_fallthrough(arena::Arena* a, const u8[]m) {
    ...
    body[0] = sw;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq(g.blocks.data[0].term.switch_arms.len, (u64)2, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_arms[0].target, (u64)3, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[0].term.switch_arms[1].target, (u64)3, m)) { return -3; }
    if(!testing::expect_eq(g.blocks.data[3].predecessors.data.len, (u64)1, m)) { return -4; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)2, m)) { return -5; }
    return 0;
}
...
fn i32 if_else_if_chain(arena::Arena* a, const u8[]m) {
    ...
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.kind, (u64)cfg::TermKind::CondBranch, m)) { return -1; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.then_target, (u64)5, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.else_target, (u64)6, m)) { return -3; }
    if(!testing::expect_eq((u64)g.blocks.data[7].term.goto_target, (u64)4, m)) { return -4; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)2, m)) { return -5; }
    return 0;
}
...
fn i32 for_break(arena::Arena* a, const u8[]m) {
    ...
    body[0] = fr;
    cfg::Cfg* g = cfg::build_cfg(mm, mk_fn(a, null, mk_block(a, &body[0], 1)));
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)5, m)) { return -1; }
    return 0;
}
...
fn i32 defer_on_continue(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq(g.blocks.data[3].stmts.data.len, (u64)1, m)) { return -1; }
    if(!testing::expect_eq((void*)g.blocks.data[3].stmts.data[0], (void*)s, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)2, m)) { return -3; }
    return 0;
}
...
fn i32 defer_with_control_flow(arena::Arena* a, const u8[]m) {
    ...
    for(u64 bi = 0; bi < g.blocks.data.len; bi += 1) {
        if((u32)g.blocks.data[bi].term.kind == (u32)cfg::TermKind::CondBranch) { has_cond = true; }
        for(u64 si = 0; si < g.blocks.data[bi].stmts.data.len; si += 1) {
            if((u16)g.blocks.data[bi].stmts.data[si].h.kind == (u16)ast::AstKind::IfStmt) { if_is_leaf = true; }
        }
    }
...
fn i32 if_then_returns_else_falls(arena::Arena* a, const u8[]m) {
    ...
    if(!testing::expect_eq((u64)g.blocks.data[3].term.kind, (u64)cfg::TermKind::Goto, m)) { return -2; }
    if(!testing::expect_eq((u64)g.blocks.data[3].term.goto_target, (u64)4, m)) { return -3; }
    if(!testing::expect_eq(g.blocks.data[4].predecessors.data.len, (u64)1, m)) { return -4; }
    return 0;
}
