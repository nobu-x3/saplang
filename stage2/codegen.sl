...
fn void declare_decl(CG* cg, u32 index) {
    ...
    if(d.kind == sapir::SapirDeclKind::Fn) {
        ...
        // COFF dedups a linkonce_odr clone only through a COMDAT; ELF manages on linkage alone.
        if(linkage == llvm::LinkOnceODRLinkage) { llvm::LLVMSetComdat(val, llvm::LLVMGetOrInsertComdat(cg.llvm_module, cstr(cg.allocator, d.link_name))); }
        ...
        if(d.is_thread_local) {
            ...
            // COFF initializes each thread from a .tls$ template, which the local-exec model bypasses.
            comprun if (build::os != "windows") { llvm::LLVMSetThreadLocalMode(val, llvm::LocalExecTLSModel); }
        }
        cg.decl_map[index] = val;
    }
}
